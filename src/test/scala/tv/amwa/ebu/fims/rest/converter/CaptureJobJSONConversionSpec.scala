...
class CaptureJobJSONConversionSpec extends WordSpec with Matchers {
   val printer = new scala.xml.PrettyPrinter(1000, 2)
  
  "A manually created capture job " should {

    "be serialized to legal JSON" in {
      val captureJob = CaptureJobXMLConversionSpec.captureJob

      val jsonOut = FimsJSON.write(captureJob)
      scala.util.parsing.json.JSON.parseFull(jsonOut).isDefined should be (true)
    }
    
    "be serialized to JSON and de-serialized back to a case class" in {
      val captureJob = CaptureJobXMLConversionSpec.captureJob

      val jsonOut = FimsJSON.write(captureJob)
      println(jsonOut)
      
      val nodesIn = JSONConverters.fromJSON(jsonOut)
      println(printer.format(nodesIn))

      val readJob = FimsJSON.read[CaptureJobType](jsonOut)
      println(readJob)
      
      readJob should equal (captureJob)
    }
  }
}